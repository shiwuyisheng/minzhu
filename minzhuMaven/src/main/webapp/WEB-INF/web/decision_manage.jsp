...
<%-- <table class="layui-table" lay-data="{width: 892, height:542, url:'${pageContext.request.contextPath}/user/showUser.do' ,toolbar: '#toolbarDemo', page:true, id:'idTest'}" lay-filter="demo">
 --%> <script type="text/html" id="toolbarDemo">
  ...
  <button class="layui-btn layui-btn-sm" id="add" lay-event="add" data-type="reload" >
   ...
   <button class="layui-btn layui-btn-sm" id="jisuan" lay-event="jisuan" data-type="reload" >
         统计决策结果
    </button>
 </div>
</script>
...
<!-- 
 {{# var today = new Date();
	if(today<=d.endDate&&today>=d.startDate){ }}
...
layui.use('table', function(){
  ...
  table.on('toolbar(demo)', function(obj){
    ...
    switch(obj.event){
     ...
     case 'jisuan':
        ...
        type: "POST",//方法类型
                dataType: "json",//预期服务器返回的数据类型
                url: "${pageContext.request.contextPath}/decision/jisuan.do" ,//url
                //data: {"id":data.id},
                success: function (result) {
                window.location.href = location.href;                
  ...
  table.on("tool(demo)", function(obj){
    ...
    if(obj.event === 'detail'){
       ...
       window.location.href="${pageContext.request.contextPath}/decision/getDecision.do?id="+data.id + "&flag=1";
     ...
     // layer.alert('编辑行：<br>'+ JSON.stringify(data));
      window.location.href="${pageContext.request.contextPath}/decision/getDecision.do?id="+data.id + "&flag=0";
    }
  });
...
<style>
  ...
  var tableModel = table.render({
    ...
    ,width: 992
     ,toolbar: '#toolbarDemo'
    ,url: '${pageContext.request.contextPath}/decision/getAll.do' //数据接口
    ,page: true //开启分页
   ...
   {type:'checkbox', fixed: 'left'}
    /*  ,{field:'id', width:80, sort: true, fixed: true} */
     ...
     // ,{field:'idnumber', title:'日期',width:120, sort: true}
      ...
      ,{field:'startDate',title:'开始日期', sort:true,templet:function (d) {
 return dateToStr(d.startDate);
 }}
      ,{field:'endDate',title:'结束日期', sort:true,templet:function (d) {
 return dateToStr(d.endDate);}}
  ...
  var table = layui.table;
  table.reload('demo',{
    url:'${pageContext.request.contextPath}/decision/getAll.do?keyword=' + keyword //,

...
<button class="layui-btn layui-btn-sm" id="add" lay-event="jisuan" data-type="reload" >
      统计选举结果
    </button>
 </div>
</script>
...
<script type="text/html" id="barDemo">
 ...
 <a class="layui-btn layui-btn-xs layui-bg-green" lay-event="edit">修改日期</a>

</script><!-- 
{{# var today = new Date();
	if((today<=d.endDate&&today>=d.startDate)||d.startDate == null){ }}
...
layui.use('table', function(){
  ...
  table.on("tool(demo)", function(obj){
    var data = obj.data;  
    if(obj.event === 'detail'){
       window.location.href="${pageContext.request.contextPath}/elect/getElection.do?id="+data.id + "&flag=1";
      ...
      layer.confirm('确认删除？', { offset: '100px',skin: 'layui-layer-molv'},function(index){
        ...
        type: "POST",//方法类型
                dataType: "json",//预期服务器返回的数据类型
                url: "${pageContext.request.contextPath}/elect/deleteElectionByid.do?id="+data.id ,//url
     ...
     // layer.alert('编辑行：<br>'+ JSON.stringify(data));
      window.location.href="${pageContext.request.contextPath}/elect/getElection.do?id="+data.id + "&flag=0";
    }
  });
...
<style>
  ...
  var tableModel = table.render({
    ...
    ,width: 992
     ...
     ,toolbar: '#toolbarDemo'
    ,url: '${pageContext.request.contextPath}/elect/getAll.do' //数据接口
    ,page: true //开启分页
   ...
   {type:'checkbox', fixed: 'left'}
    /*  ,{field:'id', width:80, sort: true, fixed: true} */
     ...
     // ,{field:'idnumber', title:'日期',width:120, sort: true}
       ,{field:'year',title:'年份'}
      ,{field:'startDate',title:'开始选举日期', sort:true,templet:function (d) {
 return dateToStr(d.startDate);
 }}
      ,{field:'endDate',title:'结束选举日期', sort:true,templet:function (d) {
 return dateToStr(d.endDate);}}
 	,{field:'post',title:'选举职位'}

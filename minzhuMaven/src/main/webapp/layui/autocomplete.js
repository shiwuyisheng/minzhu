layui.define(['jquery', 'laytpl', 'layer'], function (e) {
    "use strict";
   // layui.link('/css/autocomplete.css');
    var hint = layui.hint(),
        $ = layui.jquery,
        laytpl = layui.laytpl,
        layer = layui.layer,
        ...
        system = {
            config: {
                ...
                url:'/minzhuMaven/user/getAutoUserInfo.do',
                template_txt: '{{d.text}}',
    ...
    job.prototype.pullData = function () {
        ...
        (!_config.cache && _config.ajax[_self.index] != undefined) && _config.ajax[_self.index].abort(), _config.ajax[_self.index] = $.ajax({
            ...
            error: function () {
            	console.log("error")
            	//console.log(_config.url);console.log(_config.filter);
            },
            complete: function () {
    ...
    system.init = function (e, c) {
        ...
        _elems.each(function (_i, _e) {
            ...
            try {
                ...
                return hint.error("autocomplete" + _lay_data)
            }
            var _config = $.extend({elem: this}, system.config, c, _lay_data);
            _config.url == undefined && (_config.data == undefined || _config.length === 0) ;//&& hint.error("autocomplete");
